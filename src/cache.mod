...
// Current capacity of the PC table
uint32_t myPC_counter = 0;
// PC Predictor table array for PCs
uint64_t myPC_array[PC_TBL_SIZE];
// PC Predictor table array for policy counter
uint8_t PC_policy_counter[PC_TBL_SIZE];
// PC Predictor table array for reference
uint16_t PC_ref[PC_TBL_SIZE];

//global policy counter
uint16_t glbl_policy = MAX_GLOBAL_POLICY/2 - 1;

// Implementation of Check if address in PC array
int checkPCArray(uint64_t addr)
{
    for ( uint32_t i = 0; i < myPC_counter ; ++i)
    {
        if (addr == myPC_array[i])
        {
            return i;
        }
    }
...
//// CHECK IF IN TRACKER SET ////
int checkTrackerSet(uint32_t set)
{
    if (set < LLC_TRACKER_BOUND)
    {
        // its in tracker set
        return 1;
    }
...
// Implementation of the corresponding fn
// This function is called only when the PC table is full
int lowRefPC()
{
    int m = 0;
    for ( uint32_t i = 1; i < myPC_counter ; ++i)
    {
        if (PC_ref[m] > PC_ref[i])
            m = i;
    }
    return m;
}
...
{
    // LLC bypass if needed
    // Check if it is in PC table
    int indx = checkPCArray(addr);
    if (indx != -1)
    {
        // Use the table policy
        if(PC_policy_counter[indx] & POLICY_THRESH)
            return 1;
    }
    else
    {
        // Use global policy
        if(glbl_policy & GLOBAL_THRESH)
            return 1;
    }
    return 0;
}
...
////////////////////////////////////// HANDLE FILL ////////////////////////////////////////
// If cache is full, how to place the entry?
...
///////////////////////////////////////////////////////////////////////            
            ...
            int bypass = 0;
            if (cache_type == IS_LLC) {
                bypass = checkBypass(MSHR.entry[mshr_index].ip);
                if(bypass == 0)
                {
...
////////////////////////////////////////////////////////////
                    ...
                    if (cache_type == IS_LLC) {
                        // Check for bypass
                        bypass = checkBypass(WQ.entry[index].ip);
                        if(bypass == 0)
                        {
            ...
            if (way >= 0) { // read hit
        		...
        		if (RQ.entry[index].type == LOAD) {
        			...
        			cpu = 0;
                        ...
                        if (cache_type == IS_LLC) 
                        {
                            int indx = checkPCArray(RQ.entry[index].ip); // Check if ip or address
                            if(checkTrackerSet(set) == 1)
                            {
                                if(glbl_policy > 0)
                                    --glbl_policy;
                                if(indx != -1)
                                {
                                    if(PC_ref[indx] < REF_MAX)
                                        ++PC_ref[indx];
                                    if (PC_policy_counter[indx] > 0)
                                        --PC_policy_counter[indx];
                                }
                                else
                                {
                                    // If PC table is full
                                    if (myPC_counter == PC_TBL_SIZE)
                                    {
                                        int evict_indx = lowRefPC();
                                        myPC_array[evict_indx] = RQ.entry[index].ip;
                                        PC_policy_counter[evict_indx] = INITIAL_POLICY;
                                        PC_ref[evict_indx] = 1;
                                    }
                                    else
                                    {
                                        myPC_array[myPC_counter] = RQ.entry[index].ip;
                                        PC_policy_counter[myPC_counter] = INITIAL_POLICY;
                                        PC_ref[myPC_counter] = 1;
                                        myPC_counter++;
                                    }
                                }
                            }
...
////////////////////////////////////////////////////////////////////////////////////////
            //////// NO MSHR FOR LLC //////////
    		  if(cache_type == IS_LLC)
    		    {
                    // Update the entry in PC predictor table
                    int indx = checkPCArray(RQ.entry[index].ip); // Check if ip or address
                    if(checkTrackerSet(set) == 1)
                    {
                        if (glbl_policy < MAX_GLOBAL_POLICY)
                            ++glbl_policy;
                        if(indx != -1)
                        {
                            if(PC_ref[indx] < REF_MAX)
                                ++PC_ref[indx];
                            if (PC_policy_counter[indx] < POLICY_MAX_VAL)
                                ++PC_policy_counter[indx];
                        }
                        else
                        {
